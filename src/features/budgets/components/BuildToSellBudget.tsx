...
export const BuildToSellBudget: React.FC = () => {
  ...
  const handleSave = useCallback(() => {
    localStorage.setItem('bts_budget_draft', JSON.stringify({ projectInfo, categories, productMix }));
    toast({ title: 'Budget Saved', description: 'Your Build-to-Sell budget has been saved as a draft.' });
  }, [projectInfo, categories, productMix, toast]);

  const handleExport = useCallback(() => {
    const data = { projectInfo, categories, productMix };
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `bts_budget_${projectInfo.projectName.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.json`;
    link.click();
    toast({ title: 'Export Complete', description: 'Build-to-Sell budget exported successfully.' });
  }, [projectInfo, categories, productMix, toast]);
