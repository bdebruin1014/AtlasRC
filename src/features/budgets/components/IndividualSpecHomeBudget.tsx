...
export const IndividualSpecHomeBudget: React.FC = () => {
  ...
  const handleSave = useCallback(() => {
    localStorage.setItem('spec_home_budget_draft', JSON.stringify({ projectInfo, planSelection, profitAnalysis, categories }));
    toast({ title: 'Budget Saved', description: 'Your spec home budget has been saved as a draft.' });
  }, [projectInfo, planSelection, profitAnalysis, categories, toast]);

  const handleExport = useCallback(() => {
    const data = { projectInfo, planSelection, profitAnalysis, categories };
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `spec_home_budget_${projectInfo.projectName.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.json`;
    link.click();
    toast({ title: 'Export Complete', description: 'Spec home budget exported successfully.' });
  }, [projectInfo, planSelection, profitAnalysis, categories, toast]);
